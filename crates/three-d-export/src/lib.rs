//! Deterministic production export adapters for normalized `3d-lab` scenes.
//!
//! Scene semantics stay in `three-d-scene`; this crate owns canonical serialization policy.
...
pub fn export_scene_glb(scene: &SceneSnapshot) -> Result<Vec<u8>, SceneExportError> {
    ...
    let document = json!({
        "accessors": accessors,
        "asset": {
            "generator": "three-d-export canonical scene exporter v1",
            "version": "2.0",
        },
...
mod tests {
    ...
    fn canonical_glb_round_trips_normalized_geometry_through_loader() {
        ...
        let asset = three_d_formats::load_gltf(&bytes).unwrap();
